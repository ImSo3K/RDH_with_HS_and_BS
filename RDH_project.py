...
from PyQt5.QtWidgets import (QApplication, QWidget, QPushButton, QLabel,
                             QFileDialog, QHBoxLayout, QDesktopWidget, QVBoxLayout, QSizePolicy)
...
class App(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'Histogram Shifting & Bit Plane Slicing'
...
# image convert
    ...
    def black_and_white(self, input_image_path, output_image_path='./temp/in_wm.png'):
        ...
        self.image3.setPixmap(QPixmap(
            './temp/embeded.png').scaled(280, 180, Qt.IgnoreAspectRatio, Qt.FastTransformation))
...
# image input
    def in_img(self, address=''):
        if(address == ''):
            ...
            print(im.filename)
            self.image1.setPixmap(QPixmap(self.loc1).scaled(
                640, 360, Qt.IgnoreAspectRatio, Qt.FastTransformation))
            pix = im.load()
            ...
            self.image2.setPixmap(QPixmap(
                './temp/shift.png').scaled(640, 360, Qt.IgnoreAspectRatio, Qt.FastTransformation))
        else:
            im = Image.open(self.loc1)
            ...
            psnr = cv2.PSNR(cv2.imread(self.loc1),
                            cv2.imread('./temp/embeded.png'))
            psnr = round(psnr, 4)
            self.g3.plot(x=x, y=frequency, color='lime', title=(
                'after writing data    '+'psnr='+str(round(Psnr(self.loc1, './temp/embeded.png'), 4))))
...
# image output
    def out_img(self, address='./temp/embeded.png'):
        ...
        self.g4.plot(x=x, y=frequency, color='lime', title=(
            'after restoration    '+'psnr='+str(round(Psnr(self.loc1, './temp/restored.png'), 4))))
# event handeling
    ...
    def on_click2(self):
        if self.loc1 != '':
            ...
            if fname[0]:
                ...
                self.image4.setPixmap(QPixmap(
                    './temp/restored.png').scaled(280, 180, Qt.IgnoreAspectRatio, Qt.FastTransformation))
